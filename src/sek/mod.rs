...
/// A complete Remnant Hint Message that could be found in Sekiro. Consists of
///     either one or two parts. If there are two, there will also be a
///     Conjunction string to join them.
pub struct MessageSek {
    p1: Segment,
    p2: Option<(&'static str, Segment)>,
}

impl DsMulti for MessageSek {
    /// Create a `MessageSek` with two parts.
    ...
    /// Create a `MessageSek` with one part.
